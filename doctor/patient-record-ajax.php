<?php
// doctor/patient-record-ajax.php
// Returns one patient's full record as JSON, for the Patient Records
// modal. Read-only, doctor-only, same access rules as patient-records.php
// (a doctor can look up any patient, not just their own).
//
// Blood Type, Civil Status, Emergency Contact (name + number), Allergies,
// and Current Medications are tracked in the dedicated `patient_profiles`
// table (one row per patient, created automatically the first time the
// patient saves their Profile page) and joined in below. If a patient
// hasn't saved a profile yet, the LEFT JOIN simply yields nulls and the
// frontend shows "Not on file" for those fields, same as before.
//
// SCHEMA GAP NOTE: "Existing Medical Conditions" and per-appointment
// Diagnosis still have no backing column of their own. Diagnosis is the
// one partial exception: if a consultation exists for that appointment,
// its `findings` text is shown as the diagnosis, since that's the closest
// real data already being captured.
